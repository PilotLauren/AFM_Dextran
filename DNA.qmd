...
AFM_search <- read_tsv("~/AFM_Dextran/DNA 2024-02-29/data_400[nm_ s]_7.txt", skip_empty_rows = TRUE, skip = 5, col_names = c("extension","force_f", "retraction", "force_r"))
...
AFM_normal <- AFM_DNA|>
  mutate(force_r = force_r + zero_point)|>
  mutate(force_f = -force_f + zero_point)|>
  mutate(extension = (-extension + 5314.303)- (force_r/52.82428))|>
  mutate(retraction = ((retraction + 5324.303) - (force_r/52.82428)))|>
  mutate(rownum = row_number(retraction))
...
linear <- AFM_b |>
  filter(rownum > 1500 & rownum < 1800)
```
...
max_force <- max(DNA_AFM_total$force_r)

DNA_max <- DNA_AFM_total |>
  filter(force_r == max_force)

DNA <- DNA_AFM_total|>
  filter(retraction > 0 & retraction <= 60)#|>
  #mutate(force_r = force_r - force_f)
...
write.csv(DNA, "/home/guest/AFM_Dextran/data1/DNA_good.csv", row.names=FALSE)
